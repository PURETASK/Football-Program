...
class PlayDesignApiTests(unittest.TestCase):
    ...
    def test_design_registry_persistence_review_branch_and_publish(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            status, variant_batch = handle_request(method="POST", path="/v1/playbook/designs/variants", headers=coach, body={"organization_id":"ORG-DESIGN-API", "design_id":design_id, "variants":[{"label":"Cover 3","patch":{"coverage":"cover_3"}},{"label":"Quarters","patch":{"coverage":"quarters"}}]}, service=service)
            self.assertEqual(status, 201)
