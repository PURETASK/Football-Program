...
class PlayDesignApiTests(unittest.TestCase):
    ...
    def test_design_registry_persistence_review_branch_and_publish(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            status, filtered_templates = handle_request(method="GET", path="/v1/playbook/designs/templates?organization_id=ORG-DESIGN-API&unit=offense&formation=shotgun_trips&q=flood", headers=coach, service=service)
            self.assertEqual(status, 200)
            self.assertEqual([item["id"] for item in filtered_templates["data"]["templates"]], ["TPL-OFF-FLOOD-TRIPS"])
