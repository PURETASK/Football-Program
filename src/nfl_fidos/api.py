...
def handle_request(*, method: str, path: str, body: dict[str, Any] | None = None, service: FootballIntelligenceService | None = None, headers: dict[str, str] | None = None) -> tuple[int, dict[str, Any]]:
    ...
    if parsed.path in {"/v1/playbook/designs/assets", "/v1/playbook/designs/templates", "/v1/playbook/designs/templates/lineage-impact", "/v1/playbook/designs/rule-profiles", "/v1/playbook/designs/position-options"} and method.upper() == "GET":
        ...
        if parsed.path.endswith("/position-options"):
            position = query.get("position", [""])[0]
            unit = query.get("unit", [""])[0]
            if not position or not unit:
                return 400, _response("error", None, "position and unit query parameters are required")
            try:
                return 200, _response("ok", registry.position_options(position=position, unit=unit, formation=query.get("formation", [None])[0], personnel=query.get("personnel", [None])[0], rule_profile=query.get("rule_profile", [None])[0], limit=int(query.get("limit", [24])[0])))
    ...
    if parsed.path == "/v1/playbook/designs/variants/request-review" and method.upper() == "POST":
        ...
        if denial:
            return denial
        try:
            result = _play_designs(service, organization_id=principal.organization_id, actor=principal.subject).request_variant_batch_review(body["batch_id"], actor=principal.subject, decision_ref=body["decision_ref"])
        except (KeyError, TypeError, ValueError) as exc:
            return 422, _response("invalid", None, str(exc))
        for design_id in result.get("variant_ids", []):
            _play_design_collaboration(service, organization_id=principal.organization_id, actor=principal.subject).record_event(design_id=design_id, event_type="variant_review_requested", actor=principal.subject, payload={"batch_id": result.get("id"), "decision_ref": body["decision_ref"]})
        return 200, _response("ok", result)
    if parsed.path == "/v1/playbook/designs/variants/approve-review" and method.upper() == "POST":
        required = ("organization_id", "batch_id", "decision_ref")
        missing = [field for field in required if not body.get(field)]
        if missing:
            return 400, _response("error", None, f"Missing required fields: {', '.join(missing)}")
        ...
        if denial:
            return denial
        if principal.role != "program_owner":
            return 403, _response("error", None, "Only a program_owner may approve a variant batch")
        try:
            result = _play_designs(service, organization_id=principal.organization_id, actor=principal.subject).approve_variant_batch_review(body["batch_id"], actor=principal.subject, decision_ref=body["decision_ref"])
        except (KeyError, TypeError, ValueError) as exc:
            return 422, _response("invalid", None, str(exc))
        for design_id in result.get("variant_ids", []):
            _play_design_collaboration(service, organization_id=principal.organization_id, actor=principal.subject).record_event(design_id=design_id, event_type="variant_batch_approved", actor=principal.subject, payload={"batch_id": result.get("id"), "decision_ref": body["decision_ref"]})
        return 200, _response("ok", result)
    if parsed.path == "/v1/playbook/designs/variants/create-release-bundle" and method.upper() == "POST":
        required = ("organization_id", "batch_id", "decision_ref")
        missing = [field for field in required if not body.get(field)]
        if missing:
            return 400, _response("error", None, f"Missing required fields: {', '.join(missing)}")
        ...
        if denial:
            return denial
        if principal.role != "program_owner":
            return 403, _response("error", None, "Only a program_owner may create a variant release bundle")
        try:
            result = _play_designs(service, organization_id=principal.organization_id, actor=principal.subject).create_variant_release_bundle(body["batch_id"], actor=principal.subject, decision_ref=body["decision_ref"])
        except (KeyError, TypeError, ValueError) as exc:
            return 422, _response("invalid", None, str(exc))
