...
This is a static contract check. It does not contact a registry, deploy a service, use production secrets, or enable production. The GitHub Actions `container` job builds the production image on every validated change without publishing it, then runs a production-configured health smoke test with a synthetic CI-only secret and temporary persistent volume. A provider deployment remains a separate deployment-owner action.
