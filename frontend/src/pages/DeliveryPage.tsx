...
import { buildDeliveryAgenda } from '../lib/deliveryAgenda';
...
export function DeliveryPage() {
  ...
  const agenda = useMemo(() => buildDeliveryAgenda(tasks), [tasks]);
        ...
        <WorkbenchState connected={Boolean(session)} error={dataQuery.error} loading={dataQuery.isLoading}>
          <div className="workbench-body">
            ...
            {tab === 'agenda' ? <>
              ...
              <section className="delivery-agenda" aria-labelledby="delivery-agenda-heading">
                <div className="workbench-pane__header"><div><h3 id="delivery-agenda-heading">Week timeline</h3><p>Deadlines are grouped by day so staff can see the delivery cadence, overdue work, and ownership before opening a task.</p></div><span className="workbench-form__hint">{agenda.length} day{agenda.length === 1 ? '' : 's'} · {tasks.length} task{tasks.length === 1 ? '' : 's'}</span></div>
                {agenda.length ? <div className="delivery-agenda__grid">{agenda.map((group) => <article className={`delivery-agenda__day${group.overdueCount ? ' delivery-agenda__day--overdue' : ''}`} key={group.key}><header><div><span className="eyebrow">{group.key === 'unscheduled' ? 'Planning gap' : group.key}</span><h4>{group.label}</h4></div><span className="delivery-agenda__count">{group.openCount} open{group.overdueCount ? ` · ${group.overdueCount} overdue` : ''}</span></header><ol>{group.tasks.map((task) => <li key={task.id}><button className="delivery-agenda__task" onClick={() => setSelectedId(task.id)} type="button"><span className={`delivery-agenda__dot delivery-agenda__dot--${task.computed_state || task.status || 'open'}`} aria-hidden="true" /><span><strong>{task.title}</strong><small>{task.due_at ? dueLabel(task) : 'No due time'} · {task.owner || 'Unassigned'} · {sentenceCase(task.category)}</small></span></button></li>)}</ol></article>)}</div> : <p className="record-list__empty">No tasks match this week or search filter.</p>}
              </section>
