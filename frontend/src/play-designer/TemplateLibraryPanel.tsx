    ...
    <div className="template-library">
      ...
      {variantBatches.length ? <section className="template-capture template-variant-history" aria-label="Persisted variant history">
        ...
        <div className="variant-history-list">{variantBatches.map((batch) => <article className="variant-history-card" key={batch.id}>
          ...
          {batch.release_bundle ? <div className="variant-history-card__bundle" role="status"><strong>Frozen release bundle</strong><span>{batch.release_bundle.id}</span><small>Immutable manifest · {batch.release_bundle.manifest_hash?.slice(0, 12)}… · production activation disabled</small></div> : null}
