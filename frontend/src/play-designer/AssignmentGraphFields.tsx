...
import { ROUTE_BREAKS, ROUTE_FAMILIES, ROUTE_FINISHES, ROUTE_OPTION_RULES, routeAuthoringPatch, routeBranchConstructionPatch, routeBranchGeometryPatch, routeBranchPointRemovalPatch, routeConstructionPatch } from './routeAuthoring';
    ...
    {isRoute ? <fieldset className="route-authoring-editor">
      ...
      <div className="route-branch-editor">
        ...
        {(element.branches ?? []).map((branch, index) => <div className="route-branch-row" key={branch.id}>
          ...
          <label className="inspector-field"><span>Path family</span><select aria-label={`Path ${index + 1} route family`} value={branch.route_family ?? element.route_family ?? ''} onChange={(event) => onElement(element.id, routeBranchConstructionPatch(element, design, branch.id, { route_family: event.target.value || undefined }))}><option value="">Inherited / unspecified</option>{ROUTE_FAMILIES.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label>
          <label className="inspector-field"><span>Path break</span><select aria-label={`Path ${index + 1} break`} value={branch.break_type ?? element.break_type ?? ''} onChange={(event) => onElement(element.id, routeBranchConstructionPatch(element, design, branch.id, { break_type: event.target.value || undefined }))}><option value="">Inherited / unspecified</option>{ROUTE_BREAKS.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label>
          <CommitInput label={`Path ${index + 1} stem depth (yards)`} type="number" min={0} max={60} value={branch.stem_depth_yards} onCommit={(value) => onElement(element.id, routeBranchConstructionPatch(element, design, branch.id, { stem_depth_yards: value.trim() ? Number(value) : undefined }))} />
          <CommitInput label={`Path ${index + 1} break depth (yards)`} type="number" min={0} max={60} value={branch.break_depth_yards} onCommit={(value) => onElement(element.id, routeBranchConstructionPatch(element, design, branch.id, { break_depth_yards: value.trim() ? Number(value) : undefined }))} />
          <label className="inspector-field"><span>Path finish</span><select aria-label={`Path ${index + 1} finish`} value={branch.finish_direction ?? element.finish_direction ?? ''} onChange={(event) => onElement(element.id, routeBranchConstructionPatch(element, design, branch.id, { finish_direction: event.target.value || undefined }))}><option value="">Inherited / unspecified</option>{ROUTE_FINISHES.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label>
          <label className="inspector-field"><span>Path option rule</span><select aria-label={`Path ${index + 1} option rule`} value={branch.option_rule ?? element.option_rule ?? 'none'} onChange={(event) => onElement(element.id, routeBranchConstructionPatch(element, design, branch.id, { option_rule: event.target.value }))}>{ROUTE_OPTION_RULES.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label>
