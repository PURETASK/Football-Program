...
export function offensiveBlockingIssues(design: PlayDesign): ValidationIssue[] {
  ...
  elements.forEach((element, index) => {
    ...
    if (partnerId && ids.has(partnerId)) {
      ...
      if (!['block', 'run'].includes(partner.kind)) issues.push(blockingIssue('BLOCKING_PARTNER_KIND_INVALID', `${element.type ?? element.kind} names ${partner.id} as a partner, but that assignment is not a blocking or run surface.`, `elements[${index}].block_partner_element_id`, 'Choose a blocker or run surface as the relationship partner.'));
      if (primitive === 'combo' && partner.block_partner_element_id !== element.id) issues.push(blockingIssue('COMBO_PARTNER_NOT_RECIPROCAL', `Combo ${element.id} names ${partner.id}, but the partner does not point back to ${element.id}.`, `elements[${index}].block_partner_element_id`, 'Set the same two-way combo relationship on both blockers.', 'warning'));
    }
    ...
    if (element.protection_mode && element.protection_mode !== 'screen' && !element.protection_target_element_id) issues.push(blockingIssue('PROTECTION_TARGET_REQUIRED', `${String(element.protection_mode).replaceAll('_', ' ')} protection has no explicit threat or protected target.`, `elements[${index}].protection_target_element_id`, 'Link the blocker to the threat it owns so protection teaching and validation can follow the relationship.', 'warning'));
    if (element.protection_target_element_id && !ids.has(String(element.protection_target_element_id))) issues.push(blockingIssue('PROTECTION_TARGET_MISSING', `${element.type ?? element.kind} references a protection threat that is not in this play.`, `elements[${index}].protection_target_element_id`, 'Choose an existing defensive assignment as the protection threat.', 'error'));
  });
  return issues;
}
