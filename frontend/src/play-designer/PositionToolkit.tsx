...
import type { PlayAsset, PlayDesign, PlayPlayer, PlayPositionOptions, PlayTemplate } from '../types';
...
export function PositionToolkit({ player, design, assets, templates, positionOptions, onChooseAsset, onApplyTemplate, onMaterializeAsset }: {
  player: PlayPlayer;
  design: PlayDesign;
  assets: PlayAsset[];
  ...
  const options = (positionOptions?.assets?.length ? positionOptions.assets : positionAssetOptions(player, design, assets)).slice(0, 8);
  const suggestedTemplates = (positionOptions?.templates?.length ? positionOptions.templates : positionTemplateOptions(player, design, templates)).slice(0, 3);
  ...
  return (
    <section className="position-toolkit" aria-labelledby="position-toolkit-title">
      ...
      <p className="position-toolkit__description">{profile.description} Select an option to activate it, then draw from this player on the field. {positionOptions?.status === 'ready' ? 'Ranked for this play context by the team catalog.' : 'Using the local catalog while the team catalog is unavailable.'}</p>
      <div className="position-toolkit__options" aria-label={`${position} recommended assignment options`}>
        {options.map((asset) => { const fit = positionAssetFit(asset, design); const detailsId = `position-option-${asset.id}-details`; const details = [optionDescription(asset), asset.recommendation?.reason, ...(fit.reasons.length ? [`Review notes: ${fit.reasons.join(' ')}`] : ['Compatible with the current play context.'])].filter(Boolean).join(' '); return <div className={`position-option${fit.compatible ? '' : ' is-review'}`} key={asset.id} title={details}>
